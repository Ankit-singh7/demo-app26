...
export class UserService {
  ...
  async getToken() {
    ...
    this._imdToken = await this.http.post("https://cors-anywhere.herokuapp.com/https://ciam-sb.mars.com/auth/oauth2/realms/root/realms/Miome/access_token", body , { headers }).toPromise();
    }
    catch (error) {
 ...
 async getRequest(url,body2,headers){
  ...
  async signIn(username: string, password: string) {
    ...
    try {
       await this.http.post("https://ciam-sb.mars.com/auth/oauth2/realms/root/realms/Miome/access_token", body, { headers }).toPromise();
       this._signedIn.next(true);
    } catch (error) {
      this.infoErrorModal.displayModal("error", this.errorMessages.general);
      throw error;
